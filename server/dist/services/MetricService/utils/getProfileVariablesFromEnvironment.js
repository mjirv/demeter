...
const getProfileVariablesFromEnv = () => {
    ...
    const profileVariables = Object.fromEntries(Object.entries(process.env).filter(([key]) => key.startsWith('DBT_PROFILE_') &&
        !CREDENTIAL_KEYS.includes(key.replace('DBT_PROFILE_', ''))));
    const credentials = Object.fromEntries(Object.entries(process.env).filter(([key]) => key.startsWith('DBT_PROFILE_') &&
        CREDENTIAL_KEYS.includes(key.replace('DBT_PROFILE_', ''))));
